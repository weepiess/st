...
struct ChassisPara {
        //设置参数
        short int locationMin,locationMax;//随机数区间
        short int minMoveLen=40;//最小移动距离
        short int targetOkThre=10;//到达目标位置阈值
        //运行参数
        short int randLocationTarget=250;//目标位置
        short int targetOfStart;//起始位置
        bool lastDirectionIsPost,directionIsPost;//运行方向
        int v;//运行速度
    };
//输入底盘信息
